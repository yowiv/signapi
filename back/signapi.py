...
def taskjejz(functionId, body_json):
    body_str = (body_json).strip()
    client = "android"
    client_version = "10.4.0"
    uuid = ''.join(random.sample('0123456789abcdef0123456789abcdef0123456789abcdef', 40))
    ...
    return {"data": {"st": st,"sv": sv,"sign": sign,"body": body_str,"clientVersion": data_str,"convertUrlNew": data_str,"uuid": uuid,"functionId": functionId,"convertUrl": data_str,"client": client}}
...
@server.route('/jd/sign', methods=['post'])
def Sign():
    try:
        form_data = request.form
        if "body" in form_data and "functionId" in form_data:
